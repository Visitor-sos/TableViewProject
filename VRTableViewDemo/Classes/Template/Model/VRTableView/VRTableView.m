...
@interface VRTableView () {
    int mTotalCellsVisible;
    VRInterceptor *_dataSourceInterceptor;
...
- (CGFloat)getAngleForYOffset:(CGFloat)yOffset {
    ...
    CGFloat shift = ((int)self.contentOffset.y % 60);
    CGFloat percentage = shift / 60 ;
    
    CGFloat angle_gap = M_PI/(mTotalCellsVisible+1);
    
    int rows = 0;
    if( yOffset <0.0 ) {
        rows = fabsf(yOffset) / 60;
    }
...
- (void)setupShapeFormationInVisibleCells {
    ...
    CGFloat vRadius = (self.frame.size.height-60*2.0f)/2.0f;
...
#pragma mark Layout
- (void)layoutSubviews {
    mTotalCellsVisible = self.frame.size.height / 60;
...
- (void)setDataSource:(id<UITableViewDataSource>)dataSource {
    if( !_dataSourceInterceptor) {
        _dataSourceInterceptor = [[VRInterceptor alloc] init];
    }
